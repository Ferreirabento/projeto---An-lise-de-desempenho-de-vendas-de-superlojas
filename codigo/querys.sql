...
FROM sales_performance_analysis;
-- K = 1 + 3,3 * LOG(815) = 11 É O NUMERO DE CLASSES DE STORE_SALES --
...
-- calculando a amplitude de classes de Store_sales --
WITH sales_performance_analysis AS (
    SELECT COUNT(Store_Sales) as num,
        MAX(Store_Sales) - MIN(Store_Sales) as amplitude_total
    FROM sales_performance_analysis
    WHERE Store_Sales IS NOT null
)
